# Migration Guide: v1.x to v2.0.5

Complete guide for migrating from WebVentures Comms SDK v1.x to v2.0.5.
...
WebVentures Comms SDK v2.0.5 is a major rewrite in TypeScript with full type safety, modern framework support, and improved developer experience. While the core concepts remain the same, there are several breaking changes to be aware of.
...
v2.0.5 is written in TypeScript and requires TypeScript 5.0+ for best experience. JavaScript users can still use the package, but will miss out on type checking benefits.
...
**v2.0.5** (same structure, but with TypeScript types):
...
**v2.0.5** (enhanced with error classes):
...
v2.0.5 introduces framework-specific adapters:
...
All provider configurations from v1.x are compatible with v2.0.5. However, they now have proper TypeScript types.
...
These will be removed in v2.0.5.
...
A: Yes, v2.0.5 has reduced bundle size and optimized dependencies.
